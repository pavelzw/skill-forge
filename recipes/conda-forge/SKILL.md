...
Submit a draft PR, **always** use the PR template for the description. When the conda package ships static libraries (go, rust), don't check that box in the PR template. In your PR description, never `@`-mention anybody!
Only submit a non-draft PR on staged recipes as undrafting re-triggers CI. On feedstocks, undrafting pings people which you should avoid.
...
If the upstream source needs changes, always use git-style patches (generated by `git format-patch`), not plain `diff -u` output. They carry an author, a date, and a commit message explaining why the patch is needed, so reviewers and future maintainers can tell whether it can be dropped. Generate them from a git checkout of the exact source version:

```bash
git clone --branch <TAG> <UPSTREAM_REPO_URL> src && cd src
# edit files, then:
git commit -am "<why this patch is needed, link upstream issue/PR if any>"
git format-patch -1 -o <FEEDSTOCK>/recipe/patches/
```

If the source is not a git repo (e.g., a PyPI sdist), unpack it, `git init && git add -A && git commit -m base`, then make your commit on top. List patches in the recipe:
...
Prefer upstreaming the fix and link to the upstream PR in the commit message.
